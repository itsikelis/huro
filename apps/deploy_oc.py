...
Kp = np.ones(GO2_NUM_MOTOR) * 70.0

Kd = np.ones(GO2_NUM_MOTOR) * 3.0
...
class MoveExample(Node):
    def __init__(self):
        ...
        self.command_sub = self.create_subscription(
            JointState, "/joint_cmd", self.joint_commands_handler, 10
        )
    ...
    def control(self):
        ...
        if self.time < self.init_duration_s:
            ...
            if (
                np.linalg.norm(
                    np.array(self.last_joint_commands.velocity)
                    - np.array(self.joint_commands.velocity)
                )
                >= 15.0
            ):
                ...
                self.motors_on = 0
            ratio = self.clamp(
                (self.time - self.last_cmd_time) / self.mpc_control_dt, 0.0, 1.0
            )
            for i in range(GO2_NUM_MOTOR):
                idx = joint_order[self.joint_commands.name[i]]
                cmd = low_cmd.motor_cmd[idx]
                cmd.mode = self.motors_on
                cmd.q = (1.0 - ratio) * self.last_joint_commands.position[
                    i
                ] + ratio * self.joint_commands.position[i]
                cmd.dq = (1.0 - ratio) * self.last_joint_commands.velocity[
                    i
                ] + ratio * self.joint_commands.velocity[i]
    ...
    def low_state_handler(self, msg: LowState):
        ...
        if self.q_start is None:
            ...
            self.joint_commands.velocity = [0.0] * (GO2_NUM_MOTOR)
            self.joint_commands.effort = [0.0] * (GO2_NUM_MOTOR)
