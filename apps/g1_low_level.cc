...
public:
  G1AnkleSwingSender() : Node("g1_ankle_swing_sender"), mode_machine_(0) {
    ...
    lowstate_subscriber_ = this->create_subscription<unitree_hg::msg::LowState>(
        "/lowstate", 10, [this](unitree_hg::msg::LowState::SharedPtr message) {
          LowStateHandler(message);
        });
    //  bind to g1_ankle_swing_sender::ImuStateHandler for subscribe
    //  "secondary_imu" topic
    // imustate_subscriber_ =
    // this->create_subscription<unitree_hg::msg::IMUState>(
    //     HG_IMU_TORSO, 10, [this](unitree_hg::msg::IMUState::SharedPtr
    //     message) {
...
private:
  void Control() {
    ...
    if (ms) {
      ...
      //   double const t = time_ - duration_s_ * 2;
  ...
  //     low_command.motor_cmd.at(i).q = 0.0;
  //     low_command.motor_cmd.at(i).dq = 0.0;
  //     low_command.motor_cmd.at(i).kp = 100.;
  //     low_command.motor_cmd.at(i).kd = 10.;
  //   }
  ...
  //   if (counter_ % 500 == 0) {
  //     RCLCPP_INFO(this->get_logger(), "IMU.torso.rpy: %.2f %.2f %.2f",
  //     rpy[0],
  //                 rpy[1], rpy[2]);
  //   }
  // }
  ...
  double time_{0.0};
  double control_dt_ms_{0.002}; // [2ms]
  double duration_s_{3.0};      // [3 s]
  int32_t counter_{0};
