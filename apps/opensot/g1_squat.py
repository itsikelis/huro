...
class G1OpenSotExample(Node):
    def __init__(self):
        super().__init__("g1_opensot_example")
        ...
        # print("idx", idx)
        self.stack = (
            0.1 * self.com + 0.1 * (base % [3, 4, 5]) + 0.001 * self.posture % idx
        )
    ...
    def control(self):
        ...
        if self.time < self.init_duration_s:
            ...
            self.com_ref[2] = self.com0[2] - 0.05 + self.alpha * np.sin(3.1415 * self.t)
