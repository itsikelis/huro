...
using ImuStateMsg = HumanoidRootNode::ImuStateMsg;
using MotorStateMsg = HumanoidRootNode::MotorStateMsg;
using LowStateMsg = HumanoidRootNode::LowStateMsg;
using OdometryMsg = HumanoidRootNode::OdometryMsg;

using JointStateMsg = HumanoidRootNode::JointStateMsg;
using TransformStamped = HumanoidRootNode::TransformStamped;

using TransformBroadcaster = HumanoidRootNode::TransformBroadcaster;

HumanoidRootNode::HumanoidRootNode(Params params)
    : Node("humanoid_root_node"), params_(params) {
  ...
  lowstate_sub_ = this->create_subscription<LowStateMsg>(
      ls_topic, 10,
      std::bind(&HumanoidRootNode::LowStateHandler, this,
                std::placeholders::_1));
  odometry_sub_ = this->create_subscription<OdometryMsg>(
      odom_topic, 10,
      std::bind(&HumanoidRootNode::OdometryHandler, this,
                std::placeholders::_1));
}

void HumanoidRootNode::LowStateHandler(LowStateMsg::SharedPtr message) {
  if (params_.info_imu) {
...
void HumanoidRootNode::OdometryHandler(OdometryMsg::SharedPtr message) {
  TransformStamped tf = GenerateTransformMsg(message);
  tf_broadcaster_->sendTransform(tf);
}

TransformStamped
HumanoidRootNode::GenerateTransformMsg(OdometryMsg::SharedPtr message) {
  TransformStamped tf;
